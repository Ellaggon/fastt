...
describe("S0-2 settings hub SSR readiness contract", () => {
	it("server-renders readiness/CTA and no longer ships a lying zero skeleton", () => {
		...
		expect(hydration).not.toContain("loadDiagnosticsOnOpen")
		expect(hydration).toContain('fetch("/api/provider/settings/summary?scope=hub"')
		expect(hydration).not.toContain("Qué bloquea qué")
		expect(hydration).not.toContain("data-settings-blocking-matrix")
		expect(hydration).not.toContain("data-settings-audit")
		expect(hydration).toContain("data-settings-activation-title")
	})

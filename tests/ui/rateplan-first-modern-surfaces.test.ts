...
describe("ui/rateplan-first modern surfaces", () => {
	...
	it("crear tarifas completa oferta, precio y condiciones sin mezclar reglas al editar", () => {
		...
		expect(detail).toContain('label="Nombre visible"')
		expect(detail).not.toContain("Nombre de la Tarifa")
	...
	it("overrides admin usa RBAC central, scope real, evidencia e impacto", () => {
		...
		expect(createEndpoint).toContain("requireInternalPermission")
		expect(createEndpoint).toContain("permissionForPolicyExceptionCreate")
		expect(createEndpoint).toContain('"policy.edit"')
		...
		expect(updateEndpoint).toContain("requireInternalPermission")
		expect(updateEndpoint).toContain("permissionForPolicyExceptionMutation")
	...
	it("condiciones se operan dentro de tarifa y Multicalendario", () => {
		...
		expect(ratePlanSurface).toContain("Configurar cancelación")
		expect(ratePlanSurface).toContain("Cambiar pago")
	...
	it("tarifa presenta las condiciones en lenguaje humano y reserva trazabilidad para detalle", () => {
		...
		expect(surface).toContain("Cancelación y no presentación")
		expect(surface).toContain("Pago y garantía")
		expect(surface).toContain("Llegada y salida")
		expect(surface).toContain("Cancelación gratuita")
		expect(surface).toContain("Si no llega")
		expect(surface).toContain("Editar en alojamiento")
		expect(surface).toContain("Historial y detalle técnico")
		expect(surface).toContain("Vista calculable")
		expect(surface).toContain("snapshotVersionIds")
		expect(surface).toContain("Horarios compartidos por las tarifas del alojamiento")
		expect(surface).toContain("providerHouseRules")
		expect(surface).toContain("Configurar cancelación")
		expect(useCase).toContain("buildPolicySnapshot")
		expect(useCase).toContain("PolicyExceptionRuleRepository")
		expect(useCase).toContain("buildContractFacts")
		...
		expect(detail).toContain("isProfessionalMode={isProfessionalMode}")
	})
})

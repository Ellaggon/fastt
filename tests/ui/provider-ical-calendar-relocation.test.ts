...
describe("iCal calendar relocation", () => {
	it("makes Calendar the only operational iCal workspace", () => {
		...
		const header = read("src/components/rates/CalendarWorkspaceHeader.tsx")
		...
		expect(header).toContain("workspaceDescription")
		expect(header).toContain("fastt:calendar-mode")

...
test("workspace subnavigation uses the Calendar/iCal standard from one component", () => {
	...
	expect(calendar).toContain("fastt-tabs-inside-panel__status")
	expect(calendar).toContain('data-trust-link-status-state="not_started"')
	expect(calendar).toContain("data-calendar-mode-status")

...
describe("provider integrations summary navigation", () => {
	...
	it("keeps the summary human and moves configuration behind a dedicated route", () => {
		...
		const channexClient = read("src/lib/channel-manager/channex/channex-client.ts")
		...
		expect(sync).toContain('result.accessValidated ? "sync_tested" : "reference_checked"')
		...
		expect(connectionRoute).toContain("getProviderChannelManagerOperationalReadModel")
		...
		expect(domain).toContain("Acceso validado")
		...
		expect(vendorSmoke).toContain("createChannelManagerAdapter")
		expect(channexClient).toContain("user-api-key")

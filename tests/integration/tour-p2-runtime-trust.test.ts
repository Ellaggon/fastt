...
async function seedPrivateVariant(suffix: string) {
	...
	// Public requests must resolve an actually published product, just like the guest-facing route.
...
describe("integration/tour P2 runtime trust (review, private, cross-sell, cancel→void)", () => {
	it("closes private-request accept transition with auth + idempotency", async () => {
	...
	it("closes verified review → published moderation with auth + idempotency", async () => {
		process.env.LOCAL_QA_AUTH_ENABLED = "false"
	...
	it("records cross-sell impression/click and closes booking_attributed", async () => {
		process.env.LOCAL_QA_AUTH_ENABLED = "false"
		...
		await db
			.insert(ProductGeoPlace)
			.values({
				id: `test-primary-${hotelProductId}`,
				productId: hotelProductId,
				placeId: geoPlaceId,
				role: "primary_discovery",
				isPrimary: true,
				source: "test_fixture",
	...
	it("voids tour voucher on cancel (happy path + auth negative + idempotent cancel state)", async () => {
		process.env.LOCAL_QA_AUTH_ENABLED = "false"

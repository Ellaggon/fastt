...
import { GET as confirmationStatusGet } from "@/pages/api/booking/confirmation-status"
...
describe("integration/tour check-in + guest trip (P0 1.3–1.4 / P1 auth matrix)", () => {
	it("authorizes trip/check-in matrix and refuses non-Tour mutation", async () => {
		...
		await db.insert(TourBookingQuestion).values({
			id: questionId,
		...
		const bookingId = await withSupabaseAuthStub(
			...
			async () => {
				...
				const priceQuoteId = String(holdPayload?.priceQuote?.quoteId ?? "")
				expect(holdId).toBeTruthy()
				expect(priceQuoteId).toMatch(/^pq_[a-f0-9]{32}$/)
				await db
					.update(TourBookingQuestion)
					.set({ label: "Pregunta editada después del hold", isRequired: false } as any)
					.where(eq(TourBookingQuestion.id, questionId))

				const incompleteConfirm = await bookingConfirmPost({
					request: makeAuthedJsonRequest({
						path: "/api/booking/confirm",
						token: guestToken,
						body: { holdId, priceQuoteId, leadName: "Guest Tour" },
					}),
				} as any)
				expect(incompleteConfirm.status).toBe(422)
				expect(((await readJson(incompleteConfirm)) as any)?.error).toBe("missing_required_answers")
				...
				const confirmRes = await bookingConfirmPost({
					request: makeAuthedJsonRequest({
						...
						body: {
							...
							priceQuoteId,
							leadName: "Guest Tour",
							answers: [{ questionId, value: "Hotel Centro" }],
						},
					}),
				} as any)
				...
				const idempotentConfirm = await bookingConfirmPost({
					request: makeAuthedJsonRequest({
						path: "/api/booking/confirm",
						token: guestToken,
						body: {
				...
				expect(idempotentConfirm.status).toBe(200)
				expect(String(((await readJson(idempotentConfirm)) as any)?.bookingId ?? "")).toBe(id)
				const recovery = await confirmationStatusGet({
					request: makeAuthedJsonRequest({
						path: `/api/booking/confirmation-status?holdId=${encodeURIComponent(holdId)}`,
						token: guestToken,
						method: "GET",
					}),
					url: new URL(
						`http://localhost:4321/api/booking/confirmation-status?holdId=${encodeURIComponent(holdId)}`
					),
				} as any)
				expect(recovery.status).toBe(200)
				expect(await readJson(recovery)).toMatchObject({ state: "confirmed", bookingId: id })
				const duplicateByOtherGuest = await bookingConfirmPost({
					request: makeAuthedJsonRequest({
						path: "/api/booking/confirm",
						token: otherToken,
						body: {
							holdId,
							priceQuoteId,
							leadName: "Other Guest",
							answers: [{ questionId, value: "Otro hotel" }],
						},
					}),
				} as any)
				expect(duplicateByOtherGuest.status).toBe(409)
				expect(((await readJson(duplicateByOtherGuest)) as any)?.error).toBe(
					"HOLD_OWNED_BY_ANOTHER_USER"
				)
				const confirmedBooking = await db
					.select({ guestContactSnapshotJson: Booking.guestContactSnapshotJson })
					.from(Booking)
					.where(eq(Booking.id, id))
					.then((rows) => rows[0])
				expect(confirmedBooking?.guestContactSnapshotJson).toMatchObject({
					name: "Guest Tour",
					tourAnswers: [
						{
							questionId,

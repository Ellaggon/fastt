...
describe("Guardrail: Property Content operational semantics", () => {
	...
	it("keeps legacy variant URLs as redirects into the canonical room workspace", () => {
		...
		expect(roomWorkspace).toContain("Photo tour de la habitación")
		expect(roomWorkspace).toContain("Esta imagen aparecerá en Dónde dormirás")
		...
		expect(roomWorkspace).toContain("missing_room_photo")
	...
	it("keeps the room ficha guest-facing while reading operational context through summaries", () => {
		...
		expect(roomWorkspace).toContain("Preview huésped")
		expect(roomWorkspace).toContain("Perfil")
		expect(roomWorkspace).toContain("Fotos")
		expect(roomWorkspace).toContain("Disponibilidad")
		expect(roomWorkspace).toContain("Publicación")

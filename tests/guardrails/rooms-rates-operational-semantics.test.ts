...
describe("Guardrail: Rooms & Rates operational semantics", () => {
	...
	it("keeps restrictions as the active sellability surface with operational controls", () => {
		...
		expect(restrictions).toContain("Herramientas Pro")
		expect(restrictions).toContain("Crear regla")
		expect(restrictions).toContain('label: "Precio"')
		expect(restrictions).toContain('label: "Ventana de reserva"')
		expect(restrictions).toContain("Pausar")
		expect(restrictions).toContain("Ver calendario")
		expect(restrictions).not.toContain("Herramienta profesional de reglas de venta")
		expect(restrictions).not.toContain("Acciones por temporada")
		expect(restrictions).not.toContain("Crear conjunto avanzado")
		expect(restrictions).not.toContain("Limites operativos")
		expect(restrictions).not.toContain("data-restriction-form")
		...
		expect(restrictions).toContain("Impacto")
		expect(restrictions).not.toContain("data-impact-example")
		expect(restrictions).not.toContain("data-impact-non-effect")
		expect(restrictions).not.toContain("Search evalua estas señales")
		expect(restrictions).toContain("resolveVerticalVocabulary")
		expect(restrictions).not.toContain("buildRestrictionOperationalCopyRegistry")
	...
	it("keeps Calendar operational and moves recurring price automation into Reglas de venta", () => {
		const governance = read("src/lib/backoffice-governance.ts")
		const routes = read("src/lib/routes.ts")
		...
		expect(restrictions).toContain("Multicalendario")
		expect(restrictions).toContain("routes.ratesMultiCalendar()")
		...
		expect(multiCalendar).toContain("Reglas")
		...
		expect(multiCalendar).toContain('data-multi-calendar-action="rule"')
		...
		expect(multiCalendar).toContain("Abrir Reglas de venta")
		...
		expect(multiCalendarSurface).toContain("routes.rateRestrictions()")
		...
		expect(restrictions).toContain('action === "create"')
		expect(restrictions).toContain('action === "create-batch"')
		expect(restrictions).toContain("Selección desde Multicalendario")
		expect(calendarOperationalPanel).toContain("Reglas de venta")
		...
		expect(pricingAutomationPanel).not.toContain("<Section")
		expect(pricingAutomationPanel).toContain("¿Qué regla quieres crear?")
		expect(pricingAutomationPanel).toContain("data-pricing-automation-trigger")
		expect(pricingAutomationPanel).toContain("data-pricing-automation-popover")
		expect(pricingAutomationPanel).toContain("data-pricing-automation-close")
		...
		expect(pricingAutomationPanel).toContain("data-pricing-automation-selected-title")
		expect(calendarOperationalController).not.toContain("automationSelectedTitle.textContent")
		expect(pricingAutomationPanel).not.toContain("Seleccionada")
		expect(pricingAutomationPanel).not.toContain(
			"Se aplicará automáticamente mientras esté vigente"
		)
		expect(pricingAutomationPanel).toContain("Vista previa del impacto")
		expect(pricingAutomationPanel).toContain("Selecciona tarifa, fechas y valor")
		expect(pricingAutomationPanel).toContain("Avanzado")
		expect(pricingAutomationPanel).toContain("Prioridad entre reglas")
		expect(pricingAutomationPanel).toContain("Reglas de precio activas")
		expect(pricingAutomationPanel).toContain("Estas reglas ajustan precios")
		expect(pricingAutomationPanel).toContain("Ver detalle")
		...
		expect(pricingAutomationPanel).not.toContain("Revisión del impacto")
		expect(pricingAutomationPanel).not.toContain("Esta regla actuará automáticamente después")
		expect(pricingAutomationPanel).not.toContain("Crear regla automática de precio")
		expect(pricingAutomationPanel).not.toContain("Elige qué quieres automatizar")
		expect(pricingAutomationPanel).toContain("Eliminar regla de precio")
		expect(pricingAutomationPanel).toContain("Confirmar eliminación")
		expect(pricingAutomationPanel).toContain("Reglas de precio activas")
		expect(pricingAutomationPanel).not.toContain("Reglas automáticas activas")
		...
		expect(restrictions).toContain("/api/pricing/rules/v2/create")
		expect(restrictions).toContain("deletePriceRuleDirectly")

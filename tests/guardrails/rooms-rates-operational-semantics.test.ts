...
describe("Guardrail: Rooms & Rates operational semantics", () => {
	...
	it("keeps Calendar as an interactive single-rate workspace with contextual Pro handoffs", () => {
		...
		expect(calendar).toContain("CalendarWorkspaceHeader")
		...
		expect(subnav).not.toContain("data-calendar-mode-status")
		expect(catalog).toContain('key: "price"')
		expect(catalog).toContain('label: "Precio"')
		expect(catalog).toContain('key: "availability"')
		expect(catalog).toContain('label: "Disponibilidad"')
		expect(catalog).toContain('key: "sellability"')
		expect(catalog).toContain('label: "Venta"')
		expect(catalog).toContain('key: "conditions"')
		expect(catalog).toContain('label: "Condiciones"')
		expect(catalog).toContain("workspaceDescription")
		expect(workspace).toContain("CALENDAR_ACTION_ICONS")
		...
		expect(workspace).toContain('fetch("/api/pricing/bulk-jobs"')
		expect(workspace).toContain("PricingBulkJobOperationPanel")
		...
		expect(workspace).toContain("readySurface.conditions.summary")
		...
		expect(endpoint).toContain('responseHeaders.set("Cache-Control", "private, no-store")')

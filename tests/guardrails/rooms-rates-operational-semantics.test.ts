...
describe("Guardrail: Rooms & Rates operational semantics", () => {
	...
	it("keeps Calendar operational and moves recurring sellability rules into Multicalendario", () => {
		...
		expect(multiCalendar).toContain('candidate.classList.toggle("bg-slate-950", selected)')
		expect(multiCalendar).toContain('candidate.setAttribute("aria-pressed", String(selected))')

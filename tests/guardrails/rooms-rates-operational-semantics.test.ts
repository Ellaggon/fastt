...
describe("Guardrail: Rooms & Rates operational semantics", () => {
	...
	it("keeps Tarifas as the visible client-first surface without a duplicate hub", () => {
		...
		expect(manage).toContain("Disponibilidad")
		expect(manage).toContain("Contrato")
		expect(manage).toContain("Evaluación: próximos 30 días")
		expect(manage).toContain("Lista")
		expect(manage).toContain("Borrador")
		...
		expect(manage).toContain("reglas activas")
	...
	it("keeps conditions inside rates and Multicalendar without a standalone surface", () => {
		...
		expect(multiCalendarWorkspace).toContain("Editar contrato")
	...
	it("keeps Calendar operational and moves recurring sellability rules into Multicalendario", () => {
		...
		const multiCalendar = read("src/pages/rates/multi-calendar.astro")
		const multiCalendarWorkspace = read("src/components/rates/MultiCalendarWorkspace.tsx")
		const multiCalendarApi = read("src/pages/api/rates/multi-calendar.ts")
		...
		expect(multiCalendar).toContain("loadMultiCalendarWorkspace")
		for (const label of [
			"Precio",
			"Disponibilidad",
			"Venta",
			...
			expect(multiCalendarWorkspace).toContain(label)
		}
		expect(multiCalendarWorkspace).toContain('{ id: "rules", label: "Editar reglas" }')
		expect(multiCalendarWorkspace).toContain('{ id: "conflicts", label: "Ver conflictos" }')
		expect(multiCalendarWorkspace).toContain('{ id: "history", label: "Historial" }')
		expect(multiCalendarWorkspace).not.toContain('{ id: "rules_create"')
		expect(multiCalendarWorkspace).toContain("selectedRules")
		expect(multiCalendarWorkspace).toContain("RuleEditor")
		expect(multiCalendarWorkspace).toContain("Vigencia:")
		expect(multiCalendarWorkspace).toContain("No hay reglas en esta selección")
		expect(commercialRulesApi).toContain('"update-rule"')
		expect(commercialRulesApi).toContain('"toggle-rule"')
		expect(commercialRulesApi).toContain('"create-variant"')
		expect(commercialRulesApi).toContain("Cambia la vigencia o el valor")
		expect(commercialRulesApi).not.toContain('"duplicate-rule"')
		expect(restrictionsSurface).not.toContain("duplicateRestrictionsSurfaceRule")
		expect(multiCalendarWorkspace).not.toContain(">Duplicar<")
		expect(commercialRulesApi).toContain('"delete-rule"')
		expect(multiCalendarWorkspace).toContain('role="tablist"')
		expect(multiCalendarWorkspace).toContain("aria-selected={activeTab === tab.key}")
		expect(multiCalendarWorkspace).toContain("setActiveTab")
		expect(multiCalendarWorkspace).toContain("setSelected")
		expect(multiCalendarWorkspace).toContain("/api/pricing/rules/v2/bulk-apply")
		expect(multiCalendarWorkspace).toContain("/api/inventory/bulk-apply")
		expect(multiCalendarWorkspace).toContain('form.set("action", "create-batch")')
		expect(multiCalendarWorkspace).toContain('form.set("scopeIds"')
		expect(multiCalendarWorkspace).toContain("Ajuste de precio base")
		expect(multiCalendarWorkspace).toContain("Descuento por estadía")
		expect(multiCalendarWorkspace).toContain("Anticipación mínima")
		expect(multiCalendarWorkspace).toContain("Bloquear llegada")
		expect(multiCalendarWorkspace).toContain("Guardar regla")
		expect(multiCalendarWorkspace).toContain("Ver condiciones")
		expect(multiCalendarWorkspace).toContain("Editar reglas")
		expect(multiCalendarWorkspace).toContain("policy-assignment-saved")
		expect(multiCalendarWorkspace).not.toContain("window.location.reload")
		expect(multiCalendarWorkspace).toContain("@jsxRuntime classic")
		expect(multiCalendar).toContain("client:load")
		expect(astroConfig).toContain("fastt:react-development-runtime")
		expect(astroConfig).toContain('config.command !== "serve"')
		expect(astroConfig).toContain('"process.env.NODE_ENV": JSON.stringify("development")')
		expect(astroConfig).not.toContain("force: true")
		expect(multiCalendarApi).toContain("loadMultiCalendarWorkspace")
		expect(multiCalendarApi).toContain('"Cache-Control": "private, no-store"')
		expect(commercialRulesApi).toContain('request.headers.get("accept")')
		...
		expect(multiCalendarSurface).toContain("currentOrFutureMonth")
		expect(multiCalendarSurface).toContain("day.date >= today")
		expect(multiCalendarSurface).toContain("rangeSize === 60 ? 3 : 2")
		expect(multiCalendarWorkspace).toContain('aria-label="Mes anterior"')
		...
		expect(multiCalendarSurface).toContain("routes.ratePlanPolicies")
		expect(multiCalendarWorkspace).toContain('activeTab === "conditions"')
		expect(multiCalendarWorkspace).toContain('id: "conditions"')
		expect(multiCalendarWorkspace).toContain("Ver condiciones")
		expect(multiCalendarWorkspace).toContain('data-assignment-scope="rate_plan"')
		...
		expect(calendarOperationalPanel).toContain("Revisar contrato")
		...
		expect(calendarOperationalController).toContain('"Revisar contrato"')
		expect(calendarOperationalController).toContain('"Completar contrato"')

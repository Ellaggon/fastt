...
describe("Guardrail: Rooms & Rates operational semantics", () => {
	...
	it("keeps legacy Inventory sellability compatibility explicit and off canonical surfaces", () => {
		...
		expect(dbConfig).toContain("@deprecated ARI compatibility")
		expect(dbConfig).toContain("Canonical sellability belongs to Restriction/EffectiveRestriction")
		expect(bulkService).toContain("LEGACY_SELLABILITY_INVENTORY_OPERATIONS")
		expect(bulkService).toContain("Canonical sellability writes belong to Restrictions")
		expect(bulkPreviewApi).toContain("Deprecated ARI compatibility")
		expect(bulkApplyApi).toContain("Deprecated ARI compatibility")
		expect(updateDayApi).toContain("Commercial stop-sell belongs to Restrictions")
		expect(bulkUpdateApi).toContain("Commercial stop-sell belongs to Restrictions")
		expect(inventory).not.toContain('type: "open_sales"')
		expect(inventory).not.toContain('type: "close_sales"')
		expect(inventoryBulk).not.toContain('value="open_sales"')
		expect(inventoryBulk).not.toContain('value="close_sales"')

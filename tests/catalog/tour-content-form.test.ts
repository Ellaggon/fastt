...
describe("Tour content-form wizard", () => {
	...
	it("allows keyboard image selection and continuing with existing photos", () => {
		...
		expect(handler).toContain("...existingImages.map((image) => image.id)")
		expect(handler).toContain("Subiendo ${uploaded} de ${queue.length}")
		expect(handler).toContain('form.dataset.uploading === "true"')

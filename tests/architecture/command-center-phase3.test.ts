...
describe("command center phase 3 operational UI", () => {
	...
	it("includes keyboard navigation and accessible command feedback", () => {
		...
		expect(workspace).toContain("data-command-success")
		expect(workspace).toContain("data-case-closed")
		expect(workspace).toContain("case_not_actionable")
		expect(workspace).toContain("form.dataset.submitting")
	...
	it("persists and enforces the document-security pipeline", () => {
		const migration = read("db/migrations/2026-11-04_provider_document_processing.sql")
		const processing = read("src/lib/documents/document-processing.ts")
		const upload = read("src/lib/provider-documents.ts")
		const reveal = read("src/pages/api/admin/v1/cases/[caseId]/evidence/reveal.ts")
		const panel = read("src/components/admin/cases/CaseEvidencePanel.astro")
		for (const table of ["ProviderDocumentInspection", "ProviderDocumentProcessingJob"])
			expect(migration).toContain(`"${table}"`)
		expect(migration).toContain("ON CONFLICT")
		expect(upload).toContain("ProviderDocumentProcessingJob")
		expect(processing).toContain("DOCUMENT_ANALYSIS_GATEWAY_URL")
		expect(processing).toContain("ensureDocumentProcessingBacklog")
		expect(processing).toContain('gatewayMode?: "configured" | "disabled"')
		expect(processing).toContain("documentGatewayConfigured")
		expect(processing).toContain("document_quarantined")
		expect(reveal).toContain("assertProviderDocumentSafeToReveal")
		expect(panel).toContain("Seguridad y extracción automática")
		expect(read("vercel.json")).toContain("/api/cron/document-processing")
	})
})

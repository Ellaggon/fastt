...
## Postgres test env + tour migrations (08-19 / 08-20)

Local shells can inherit a **corrupted** `DATABASE_URL` (spaces / missing `@`). Dotenv does not override existing process env, so Vitest fails with `Invalid URL` even when `.env` is valid.

Mitigations in-repo:

- `tests/setup/clean-db-env.ts` (Vitest `setupFiles`) strips invalid URL env vars, then loads `.env` / `.env.test`
- `pnpm test:tours:phase6` also runs under `env -u DATABASE_URL -u DIRECT_URL -u SUPABASE_DB_POOLER_URL -u SUPABASE_DB_URL`
- Migration helpers call `ensureCleanPostgresEnv()` before connecting

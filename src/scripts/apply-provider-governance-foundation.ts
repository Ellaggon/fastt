...
async function run(statement: string): Promise<"applied" | "skipped"> {
	try {
		await db.run(sql.raw(statement))
		return "applied"
	} catch (error) {
		if (isAlreadyApplied(error)) return "skipped"
		throw error
	}
}

export default async function applyProviderGovernanceFoundation() {
	let applied = 0
	let skipped = 0

	for (const statement of [...providerProfileColumns, ...governanceSchema]) {
	...
	console.log(
		JSON.stringify(
			{
				migration: "provider_governance_foundation",
				applied,
				skipped,
			},
			null,

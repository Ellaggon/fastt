import type { HouseRuleRecord, HouseRuleRepositoryPort } from "../ports/HouseRuleRepositoryPort"
import {
...
function sortRows(rows: HouseRuleRecord[]) {
	rows.sort((a, b) => {
...
export async function listHouseRulesByProduct(
	...
	sortRows(rows)
	return rows.map(serializeHouseRule)
}

export async function listHouseRulesByProductIds(
	deps: { repo: HouseRuleRepositoryPort },
	productIds: string[]
): Promise<Map<string, HouseRule[]>> {
	...
	const rulesByProduct = new Map<string, HouseRule[]>()
	for (const row of rows.map(serializeHouseRule)) {

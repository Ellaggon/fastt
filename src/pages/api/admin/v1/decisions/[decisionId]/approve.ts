...
export const POST: APIRoute = async ({ request, params }) => {
	...
	try {
		if (!getFeatureFlag("COMMAND_CENTER_V2_COMMANDS_ENABLED", { request }))
			return Response.json({ error: "casework_commands_disabled" }, { status: 503 })
		...
		const audit: SensitiveCommandAudit = {
			requestId,
			action: "case.decision.approve_high_risk",
		...
		const command = await executeSensitiveCommand({
			audit,
			idempotency: {
				scope: "case.decision.approve_high_risk",
				key: idempotencyKeyFromRequest(request),
				payload: { decisionId, expectedCaseVersion, reason: body.reason ?? null },
			},
			authorize: async () => {
		...
		return withRequestId(
			Response.json(
				{ error: error instanceof Error ? error.message : "case_approval_failed" },
				{ status }
			),
			requestId
		)

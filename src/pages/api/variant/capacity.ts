...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		const minOccupancy = Number(form.get("minOccupancy"))
		const maxOccupancy = Number(form.get("maxOccupancy"))
		const maxAdultsRaw = form.get("maxAdults")
		const maxChildrenRaw = form.get("maxChildren")
		...
		const result = await setVariantCapacity(
			{ repo: variantManagementRepository },
			{
				variantId,
				minOccupancy,
				maxOccupancy,
				maxAdults: maxAdultsRaw === null || maxAdultsRaw === "" ? null : Number(maxAdultsRaw),
				maxChildren:

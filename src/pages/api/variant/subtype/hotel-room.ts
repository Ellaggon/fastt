...
import { attachHotelRoomSubtype } from "@/modules/catalog/public"
...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		const roomTypeId = String(form.get("roomTypeId") ?? "").trim()
		...
		const result = await attachHotelRoomSubtype(
		...
		const status = msg.includes("must be hotel_room") || msg.includes("already exists") ? 400 : 500

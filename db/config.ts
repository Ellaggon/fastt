...
const ProviderIntegrationConnection = defineTable({
	columns: {
		...
		syncIntervalMinutes: column.number({ default: 1440 }),
		nextSyncAt: column.date({ optional: true }),
...
const ProviderIntegrationCredential = defineTable({
	columns: {
		...
		encryptedJson: column.json(),
		scopesJson: column.json({ optional: true }),
		tokenExpiresAt: column.date({ optional: true }),
		refreshAfterAt: column.date({ optional: true }),
		lastRefreshedAt: column.date({ optional: true }),
		revokedAt: column.date({ optional: true }),
		createdAt: column.date({ default: NOW }),
		updatedAt: column.date({ default: NOW }),
	},
	indexes: [{ on: ["providerId"] }, { on: ["providerId", "tokenExpiresAt"] }],
})
const ProviderIntegrationMapping = defineTable({
	columns: {
		id: column.text({ primaryKey: true }),
		providerId: column.text({ references: () => Provider.columns.id }),
		connectionId: column.text({ references: () => ProviderIntegrationConnection.columns.id }),
		mappingType: column.text(),
		localEntityType: column.text(),
		localEntityId: column.text(),
		externalEntityType: column.text(),
		externalEntityId: column.text(),
		externalEntityName: column.text({ optional: true }),
		direction: column.text({ default: "bidirectional" }),
		status: column.text({ default: "active" }),
		metadataJson: column.json({ optional: true }),
		lastVerifiedAt: column.date({ optional: true }),
		createdAt: column.date({ default: NOW }),
		updatedAt: column.date({ default: NOW }),
	},
	indexes: [
		{ on: ["connectionId", "mappingType", "localEntityId"], unique: true },
		{ on: ["connectionId", "mappingType", "externalEntityId"], unique: true },
		{ on: ["providerId", "status"] },
	],
})
const ProviderIntegrationSyncRun = defineTable({
	columns: {
		id: column.text({ primaryKey: true }),
		providerId: column.text({ references: () => Provider.columns.id }),
		connectionId: column.text({ references: () => ProviderIntegrationConnection.columns.id }),
		...
		trigger: column.text({ default: "manual" }),
		status: column.text({ default: "running" }),
		idempotencyKey: column.text({ optional: true }),
		readCount: column.number({ default: 0 }),
		changedCount: column.number({ default: 0 }),
		skippedCount: column.number({ default: 0 }),
		failedCount: column.number({ default: 0 }),
		cursor: column.text({ optional: true }),
		errorCode: column.text({ optional: true }),
		errorMessage: column.text({ optional: true }),
		summaryJson: column.json({ optional: true }),
		requestedBy: column.text({ optional: true, references: () => User.columns.id }),
	...
	indexes: [
		...
		{ on: ["connectionId", "startedAt"] },
		{ on: ["providerId", "status", "startedAt"] },
	],
})
const ProviderIntegrationSyncJob = defineTable({
	columns: {
		id: column.text({ primaryKey: true }),
		providerId: column.text({ references: () => Provider.columns.id }),
		connectionId: column.text({
...
const ProviderIntegrationIncident = defineTable({
	columns: {
		id: column.text({ primaryKey: true }),
		providerId: column.text({ references: () => Provider.columns.id }),
		connectionId: column.text({ references: () => ProviderIntegrationConnection.columns.id }),
		syncRunId: column.text({
			optional: true,
			references: () => ProviderIntegrationSyncRun.columns.id,
		}),
		mappingId: column.text({
			optional: true,
			references: () => ProviderIntegrationMapping.columns.id,
		}),
		dedupeKey: column.text(),
		code: column.text(),
		...
		actionHref: column.text({ optional: true }),
		entityType: column.text({ optional: true }),
		entityId: column.text({ optional: true }),
		occurrenceCount: column.number({ default: 1 }),
		firstSeenAt: column.date({ default: NOW }),
		lastSeenAt: column.date({ default: NOW }),
		resolvedAt: column.date({ optional: true }),
		resolvedBy: column.text({ optional: true, references: () => User.columns.id }),
		resolutionNote: column.text({ optional: true }),
		notificationStatus: column.text({ default: "pending" }),
		notificationChannelsJson: column.json({ optional: true }),
		notificationAttemptCount: column.number({ default: 0 }),
		notifiedAt: column.date({ optional: true }),
		notificationError: column.text({ optional: true }),
		metadataJson: column.json({ optional: true }),
		createdAt: column.date({ default: NOW }),
		updatedAt: column.date({ default: NOW }),
	},
	indexes: [
		{ on: ["connectionId", "dedupeKey"], unique: true },
...
const InventoryResource = defineTable({
	columns: {
		id: column.text({ primaryKey: true }),
		providerId: column.text({ references: () => Provider.columns.id }),
	...
	indexes: [
		{ on: ["providerId", "variantId", "status"] },
		{ on: ["variantId", "label"], unique: true },
	],
})

const ProviderExternalCalendar = defineTable({
	columns: {
		id: column.text({ primaryKey: true }),
		providerId: column.text({ references: () => Provider.columns.id }),
		connectionId: column.text({
		...
		name: column.text(),
		feedUrlEncrypted: column.json(),
		feedUrlHost: column.text(),
		feedUrlFingerprint: column.text(),
		status: column.text({ default: "pending" }),
		lastSyncAt: column.date({ optional: true }),
		lastSyncStatus: column.text({ optional: true }),
		lastError: column.text({ optional: true }),
		lastEventCount: column.number({ default: 0 }),
		etag: column.text({ optional: true }),
		lastModified: column.text({ optional: true }),
		syncEnabled: column.boolean({ default: true }),
		syncIntervalMinutes: column.number({ default: 1440 }),
...
const ProviderExternalCalendarEvent = defineTable({
	columns: {
		id: column.text({ primaryKey: true }),
		...
		variantId: column.text({ references: () => Variant.columns.id }),
		resourceId: column.text({ references: () => InventoryResource.columns.id, optional: true }),
		sourceKey: column.text(),
		externalUid: column.text(),
		summary: column.text({ optional: true }),
		startDate: column.text(),
		endDate: column.text(),
		sourceUpdatedAt: column.date({ optional: true }),
	...
	indexes: [
		{ on: ["calendarId", "sourceKey"], unique: true },
		{ on: ["variantId", "isActive", "startDate", "endDate"] },
		{ on: ["resourceId", "isActive", "startDate", "endDate"] },
		{ on: ["calendarId", "isActive"] },
	],
})

const ProviderExternalCalendarConflict = defineTable({
	columns: {
		id: column.text({ primaryKey: true }),
		providerId: column.text({ references: () => Provider.columns.id }),
		calendarId: column.text({ references: () => ProviderExternalCalendar.columns.id }),
		...
		actionLabel: column.text({ optional: true }),
		resolutionNote: column.text({ optional: true }),
		actedAt: column.date({ optional: true }),
		actedBy: column.text({ references: () => User.columns.id, optional: true }),
		firstSeenAt: column.date({ default: NOW }),
		lastSeenAt: column.date({ default: NOW }),
		metadataJson: column.json({ optional: true }),
		createdAt: column.date({ default: NOW }),
	...
	indexes: [
		{ on: ["calendarId", "dedupeKey"], unique: true },
		{ on: ["providerId", "status", "lastSeenAt"] },
		{ on: ["calendarId", "status"] },
	],
})

const ProviderExternalCalendarExport = defineTable({
	columns: {
		id: column.text({ primaryKey: true }),
		providerId: column.text({ references: () => Provider.columns.id }),
		variantId: column.text({ references: () => Variant.columns.id }),
		...
		status: column.text({ default: "active" }),
		lastDownloadedAt: column.date({ optional: true }),
...
const EffectiveAvailability = defineTable({
	columns: {
		...
		externalBlockedUnits: column.number({ default: 0 }),
